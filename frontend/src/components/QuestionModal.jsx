...
export default function QuestionModal({ question, onClose }) {
  ...
  return (
    ...
    >
      <div
        className="relative w-full max-w-2xl max-h-[85vh] overflow-y-auto rounded-2xl border border-white/8 bg-[#0d1117] shadow-2xl"
        onClick={e => e.stopPropagation()}
        id="question-modal-content"
      >
        {/* Header */}
        <div className="sticky top-0 bg-[#0d1117]/95 backdrop-blur-sm border-b border-white/6 px-6 py-4 flex items-start justify-between gap-4">
          ...
          <button
            onClick={onClose}
            className="p-1 mt-1 transition-colors shrink-0 text-slate-400 hover:text-white"
            id="modal-close-btn"
          >
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-3 gap-3">
            <div className="p-3 text-center border rounded-xl bg-white/3 border-white/6">
                    ...
                    {byTimeframe[tf]
                      ...
                      .map((c, i) => (
                        <div key={i} className="flex items-center gap-3">
                          <span className="w-32 text-sm capitalize text-slate-200 shrink-0">{c.company}</span>
                          ...
                          <span className="w-10 font-mono text-xs text-right text-slate-400">
          ...
          {question.link && (
            <a
              ...
              className="flex items-center justify-center w-full gap-2 py-3 text-sm font-semibold text-white transition-all duration-200 shadow-lg rounded-xl bg-linear-to-r from-indigo-600 to-violet-600 hover:from-indigo-500 hover:to-violet-500 hover:shadow-indigo-500/25"
            >
